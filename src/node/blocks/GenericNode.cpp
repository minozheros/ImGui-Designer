...
namespace core
{
    GenericNode::GenericNode(const std::string &title, const std::vector<IParameterBase *> &params)
        : title_(title), params_(params)
    {
    }
    ...
    void GenericNode::render()
    {
        ...
        for (auto *param : params_)
        {
            ...
            if (param->getParameterType() == ParameterType::INPUT)
            {
                inputWidth = std::max(inputWidth, labelWidth);
                inputCount++;
            }
            else if (param->getParameterType() == ParameterType::RETURN)
            {
        ...
        for (auto *param : params_)
        {
            if (param->getParameterType() == ParameterType::RETURN)
            {
