...
def main(output_path=default_output):
    ...
    for func in functions:
        ...
        params_map_entries.append(f'    {{"{name}", []() -> std::unique_ptr<IParameterParamsBase> {{ return std::make_unique<{class_name}>(); }}}}')
    ...
    with open(output_path, 'w') as f:
        ...
        f.write('#include "core/types/interfaces/IParameterParamsBase.hpp"\n')
        for inc in sorted(includes):
            f.write(f'{inc}\n')
        f.write('\nnamespace core {\n')
        f.write('static const std::unordered_map<std::string, std::function<std::unique_ptr<IParameterParamsBase>()>> NODE_PARAMS_FACTORY = {\n')
